...
public class TemplateClassProcessor implements StringProcessor {
    ...
    public String process(String code) {
        ...
        while (m2.find()) {
            ...
            TemplateClass tmpCls = null;

            for (TemplateClass templateClass : templateClasses) {
                if (templateClass.getName().equals(tI.getName())) {
                    tmpCls = templateClass;
                    break;
                }
            }

            // check whether template instance is really concrete
            // concrete is defined as having no args that are equal to
            // template args
            boolean isConcrete = true;
            if (tmpCls != null) {
                String[] tmpClsArgs = tmpCls.getTemplateArguments().split(",");
                String[] tIClsArgs = tI.getTemplateArguments().split(",");

                if (tmpClsArgs.length == tIClsArgs.length) {

                    for (int i = 0; i < tmpClsArgs.length; i++) {
                        if (tmpClsArgs[i].equals(tIClsArgs[i])) {
                            isConcrete = false;
                            break;
                        }
                    }
            ...
            if (!getTemplateInstances().contains(tI) && isConcrete
